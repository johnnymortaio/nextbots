...
ENT.JumpSound = {
	Sound("npc_faraoh/faraoh_shamofaraoh.mp3")
}
ENT.JumpHighSound = {
	Sound("npc_faraoh/faraoh_shamofaraoh.mp3")
}
ENT.TauntSounds = {
	Sound("npc_faraoh/faraoh_oh.mp3")
}
ENT.ChaseSounds = {
	Sound("npc_faraoh/faraoh_sofaraoh.mp3"),
	Sound("npc_faraoh/faraoh_sofaraoh2.mp3"),
	Sound("npc_faraoh/faraoh_demidesh.mp3"),
	Sound("npc_faraoh/faraoh_end.mp3"),
}
local chaseMusic = Sound("npc_faraoh/faraoh_loop.mp3")

local workshopID = "2832996985"
...
local npc_faraoh_acquire_distance =
	CreateConVar("npc_faraoh_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which faraoh will chase a target.")

local npc_faraoh_spawn_protect =
	CreateConVar("npc_faraoh_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, faraoh will not target players or hide within 200 units of \z
	a spawn point.")

local npc_faraoh_attack_distance =
	CreateConVar("npc_faraoh_attack_distance", 80, FCVAR_NONE,
	"The reach of faraoh's attack.")

local npc_faraoh_attack_interval =
	CreateConVar("npc_faraoh_attack_interval", 0.2, FCVAR_NONE,
	"The delay between faraoh's attacks.")

local npc_faraoh_attack_force =
	CreateConVar("npc_faraoh_attack_force", 800, FCVAR_NONE,
	"The physical force of faraoh's attack. Higher values throw things \z
	farther.")

local npc_faraoh_smash_props =
	CreateConVar("npc_faraoh_smash_props", 1, FCVAR_NONE,
	"If set to 1, faraoh will punch through any props placed in their way.")

local npc_faraoh_allow_jump =
	CreateConVar("npc_faraoh_allow_jump", 1, FCVAR_NONE,
	"If set to 1, faraoh will be able to jump.")

local npc_faraoh_hiding_scan_interval =
	CreateConVar("npc_faraoh_hiding_scan_interval", 3, FCVAR_NONE,
	"faraoh will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant faraohs are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_faraoh_hiding_repath_interval =
	CreateConVar("npc_faraoh_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to faraoh's hiding spot will be redetermined every X seconds.")

local npc_faraoh_chase_repath_interval =
	CreateConVar("npc_faraoh_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of faraoh's target will be redetermined every \z
	X seconds.")

local npc_faraoh_expensive_scan_interval =
	CreateConVar("npc_faraoh_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_faraoh_force_download =
	CreateConVar("npc_faraoh_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download faraoh resources \z
...
local CHASE_SOUND_INTERVAL = 4

if npc_faraoh_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("faraoh_nag")
util.AddNetworkString("faraoh_navgen")
...
local VECTOR_faraoh_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_faraoh_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_faraoh: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other faraohs, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_faraoh"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "faraohMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_faraoh" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why faraoh isn't working.
	net.Start("faraoh_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_faraoh: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_faraoh: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_faraoh: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_faraoh: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_faraoh: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, faraoh in pairs(ents.FindByClass("npc_faraoh")) do
		faraoh:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "faraohNavGen", navEndGenerate)
	else
		print("npc_faraoh: nav_generate failed to initialize")
...
concommand.Add("npc_faraoh_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_faraoh: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. faraoh doesn't use
	...
	elseif isConsole then
		print("npc_faraoh: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_faraoh_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_faraoh_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_faraoh_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_faraoh_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. faraoh means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_faraoh_smash_props:GetBool() then continue end
...
function ENT:AtfaraohtJumpAtTarget()
	-- No double-jumping.
	...
	local maxAttackDistance = npc_faraoh_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_faraoh_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_faraoh_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_faraoh_attack_distance:GetInt()
		...
		local repathInterval = npc_faraoh_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_faraoh_allow_jump:GetBool()
			and currentTime - self.LastJumpScan >= scanInterval
		then
			self:AtfaraohtJumpAtTarget()
			self.LastJumpScan = currentTime
		end
	else
		local hidingScanInterval = npc_faraoh_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_faraoh_hiding_repath_interval:GetFloat()
...
local MAT_faraoh = Material("npc_faraoh/faraoh")
local MAT_faraoooh = Material("npc_faraoh/faraoooh")
killicon.Add("npc_faraoh", "npc_faraoh/killicon", color_white)
language.Add("npc_faraoh", "FARAOH")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_faraoh: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a faraoh.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_faraoh_music_volume =
	CreateConVar("npc_faraoh_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by faraoh. (0-1, where 0 is muted)")

-- If another faraoh comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, faraohs do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_faraoh_PANIC_COUNT faraohs are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of faraoh.
local MUSIC_faraoh_PANIC_COUNT = 8

local MUSIC_faraoh_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_faraoh_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_faraoh") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("faraohPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_faraoh_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_faraoh" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_faraoh_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear faraoh taunt us.
...
local function startTimer()
	if not timer.Exists("faraohPanicMusicUpdate") then
		timer.Create("faraohPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	...
	if ( raging ) then
		render.SetMaterial(MAT_faraoooh)
	else
		render.SetMaterial(MAT_faraoh)
	end

	-- Get the normal vector from faraoh to the player's eyes, and then compute
	...
	-- faraoh should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("faraohHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("faraohHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("faraoh is studying this map.", "faraohHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "faraohHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "faraohHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "faraohHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "faraohHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("faraoh_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "faraohNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. faraoh doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_faraoh_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for faraoh to figure \z
		this map out.\n\z
...
net.Receive("faraoh_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! faraoh doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet faraoh-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! faraoh doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to faraoh.\n\z
			\n\z
			If you ARE the server host, you can run npc_faraoh_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not faraoh-compatible!",
			"Ok", nil,
